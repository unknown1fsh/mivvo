...
export const useDamageAnalysis = () => {
  ...
  const startDamageAnalysis = useCallback(async (
    ...
    try {
      toast.loading('Hasar analizi raporu hazýrlanýyor...', { id: toastId })
      ...
      if (!reportResponse.data?.success) {
        throw new Error(reportResponse.data?.message || 'Rapor oluþturulamadý')
      }

      const reportId = reportResponse.data.data?.reportId
      if (!reportId) {
        throw new Error('Rapor ID alýnamadý')
      }

      toast.loading('Fotoðraflar yükleniyor...', { id: toastId })
      ...
      if (!uploadResponse.data?.success) {
        throw new Error(uploadResponse.data?.message || 'Fotoðraflar yüklenemedi')
      }

      toast.loading('AI analizi baþlatýldý...', { id: toastId })
      ...
      if (!analysisResponse.data?.success) {
        throw new Error(analysisResponse.data?.message || 'AI analizi tamamlanamadý')
      }

      toast.success('Hasar analizi tamamlandý!', { id: toastId })
      ...
      const message = error?.response?.data?.message || error?.message || 'Hasar analizi sýrasýnda bir hata oluþtu'
