...
import { AIAnalysisResults, VehicleInfo, PaintAnalysisResult } from '@/types/report'
...
export const usePaintAnalysis = () => {
  ...
  const simulateAIAnalysis = useCallback((vehicleInfo: VehicleInfo, uploadedImagesCount: number): AIAnalysisResults => {
    ...
    const paintAnalysis: PaintAnalysisResult = {
      ...
      scratches: Math.floor(Math.random() * 5),
      dents: Math.floor(Math.random() * 3),
      rust: Math.random() > 0.8,
      oxidation: Math.floor(Math.random() * 25), // 0-25%
      ...
      technicalDetails: {
        primerType: 'Akrilik',
        baseCoatType: 'Su bazlı',
        clearCoatType: 'Seramik',
        uvProtection: true,
        applicationMethod: 'Robotik sprey'
      }
    }
